...
ggplotly(
ggplot(perms_x_dist_sum %>% filter(year > 2013), aes(x = year, y = permits_count, color = as.character(color), group = interaction(DISTRICT, color))) +
  geom_line(lwd = 0.5) +
...
filtered_zoning %>%
  ...
  filter(rf_predictions > 10,
         n_contig > 2) %>%
  arrange(desc(rf_predictions, n_contig)) %>%
